        ...
        <v-layout row wrap>
            <v-flex xs12 sm8>
                <v-card v-if="posts.length">
                    <post-list :posts="posts" :group-name="group.name" :is-admin="isAdmin" :is-moderator="isModerator"></post-list>
                </v-card>
                <v-alert v-if="!posts.length"
                         :value="true"
                         type="info"
                >
                    No posts
                ...
                <v-layout row>
                    <!--<v-flex xs12>-->
                        <v-card>
                            ...
                            <v-btn :color="isSubbed ? 'danger' : 'success'" @click="toggleSub()" flat>{{ (isSubbed ? 'unsubscribe' : 'subscribe') | t }}</v-btn>
                            <v-btn flat :to="{name: 'postCreateView', params: {groupName: group.name}}">{{'add-post-in-group' | t}}</v-btn>
                        </v-card>
                    <!--</v-flex>-->
                </v-layout>
                <v-divider></v-divider>
